...
st.markdown("<h1>Supply Chain Analytics and Late Delivery Prediction</h1>", unsafe_allow_html=True)
...
st.subheader("Random Forest Late Delivery Prediction")
...
if st.button("Predict Late Delivery"):
    ...
    if 'model' not in st.session_state:
        ...
        input_df = pd.DataFrame(
            {
                ...
                'Days_for_shipping_(real)': [real_days],
        ...
        st.write(f"Late Delivery Probability: {late_prob:.2f}%")
        st.write(f"On-Time Probability: {ontime_prob:.2f}%")
        st.write(f"Threshold Applied: {threshold}%")
